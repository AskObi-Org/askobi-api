...
class SearchQuery:
    ...
    def parse_datetime(self, key: str) -> datetime | None:
        ...
        if (
            len(date) >= 3
            and date[0] == "-"
            and date[-1] in self.DATE_FORMATS
            and is_int(date[1:-1])
        ):
            val = int(date[1:-1])
            dt_format = date[-1]
    ...
    def get_created_filter(
        self, model: type[ModelProtocol], key: str = "created"
        ...
        if start_date:
            queries.append(
                get_sqla_attr(cast(ModelProtocol, model), "created") >= start_date
            )
        if end_date:
            queries.append(
                get_sqla_attr(cast(ModelProtocol, model), "created") <= end_date
            )
        return queries
...
def handle_event_loop_exception(
    logger: Logger, loop: asyncio.AbstractEventLoop, context: dict[str, Any]
) -> None:
    msg = (
        get_exception_message(context["exception"])
